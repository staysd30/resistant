...
public class SearchFilter extends HttpServlet {
	...
	Vector<String[]> v = DBHelper.runQuery("select * from product p INNER JOIN vendor v ON p.venId=v.id INNER JOIN category c ON p.catId=c.id where v.name LIKE '%"+search+"%' OR c.name LIKE '%"+search+"%' OR p.description LIKE '%"+search+"%'OR p.features LIKE '%"+search+"%' OR p.vendorModel LIKE '%"+search+"%' ;");
